Mealpler.component('weekManager', {
    ...
    controller: function ($scope, MealService, WeekService, $timeout) {
        this.$onInit = () => {
            ...
            this.MainCtrl.addDatabaseHandlers((data, date) => this.init(data, date, this.MainCtrl.uid));

            //manually initialize week ctrl if user is not signed in
            if (!this.MainCtrl.uid) this.init(void 0, this.startDate);
        };
        ...
        this.setWeekDaysFoodInfo = (data, id) => {
            if (data || data === null) {
                ...
                return WeekService.findDateRangeMealList(this.weekStartDate.date, this.weekDuration, id).then((response) => {
                    $timeout(() => {
